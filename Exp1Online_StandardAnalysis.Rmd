...
Exp1data_OL <- Exp1data_OL %>% select(-blocks.ran,-blocks.thisIndex)
```
...
pander(summary(aov(RT_ifo~Validity+Saliency+Validity*Saliency, data = Exp1agg_Standard_fo)),
       style = "rmarkdown", caption = "ANOVA results using aov function")
...
pander(summary(aov(RT_io~1+Validity+Saliency+Validity*Saliency, data = Exp1agg_Standard_o)),
       style = "rmarkdown", caption = "Results using aov function")
# 
...
panderOptions('table.split.table',300)
pander(anova_ER, style = 'rmarkdown', caption = "Standard ANOVA table: Error Rates as a function of Saliency and Validity", split.table = Inf)

pander(summary(aov(ErrorRate~1+Validity+Saliency+Validity*Saliency, data = aggER)),
       style = "rmarkdown", caption = "Error rate Results using aov function")
...
anova_Block_fo <- ezANOVA(data = aggBlock_fo,
                    dv = RT_ifo,
                    wid = participant,
                    within = .(Saliency,Validity,BlockCount),
                    detailed = TRUE,
...
anova_Block_o <- ezANOVA(data = aggBlock_o,
                    dv = RT_io,
                    wid = participant,
                    within = .(Saliency,Validity,BlockCount),
                    detailed = TRUE,
                    return_aov = TRUE)
#anova_out(anova_Block_o)
...
Exp1agg_Age_fo <- aggregate(data = Exp1data_OL_age, RT_ifo~participant+Saliency+Validity, mean)

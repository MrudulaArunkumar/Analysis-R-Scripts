...
Exploring the data trends by comparing the fast and slow participants based on age
```{r}
...
stdaovSV <- ggplot(aggmeanSV_fo, aes(x=Sal, color=Val, y=RTmean))+
  geom_line(aes(group=Val, linetype = Val), size = 1)+
...
ggsave(stdaovSV, file = "stdaovSV_fo.png", dpi = 800)
```
...
stdaov_o <- ggplot(aggmeanSV_o, aes(x=Sal, color=Val, y=RTmean))+
  geom_line(aes(group=Val, linetype = Val), size = 1)+
  geom_point(aes(shape = Val), size = 2)+
...
ggsave(stdaov_o, filename = "stdaovSV_o.png", dpi = 800)
```
...
```{r}
include_graphics("D:/PhD/Exp1-Overshadowing/Exp/WORD PAIR Onlinestudy.jpg")
```

Points to be noted:
This is the basic assignment given in the conditions.xlsx used in psychopy with the colour assignment of saliency based on white background
So, this colour assignment was not counterbalanced based on screen background colour, and so for the black background, what was coded as salient, appeared as non salient since the colour was marked as dark grey (which is nonsalient on a black background).

This is also probably why there was a main effect of background colour in one of the earlier analyses.

This has been corrected below:

First is to create the aggregate based on word pair of the entire data set and to see how the data looks while analysing the entire data set across the factors of word pair, word too.
There is a strong 3 way interaction between Distractor, Validity and Word pair indicating a possible effect of choice of word and validity
```{r warning=FALSE}

wordagg <- aggregate(data=Exp1data_OL, RT_ifo~participant+Validity+Distractor1+Saliency+WordPair+CorrectAnswer, mean)


wordagg$participant <- as.factor(wordagg$participant)
wordagg$Validity <- as.factor(wordagg$Validity)
wordagg$Distractor1 <- as.factor(wordagg$Distractor1)
wordagg$Saliency <- as.factor(wordagg$Saliency)
wordagg$WordPair <- as.factor(wordagg$WordPair)
wordagg$CorrectAnswer <- as.factor(wordagg$CorrectAnswer)

wordanova <- ezANOVA(data = wordagg,
                     dv = RT_ifo,
                     wid = participant,
                     within = .(Validity, Distractor1),
                     between = .(WordPair),
                     detailed = TRUE)
anova_out(wordanova)

meanWordagg <- ddply(wordagg, .(Validity,Saliency,Distractor1,WordPair,CorrectAnswer), summarize, RTmean=mean(RT_ifo), SD = sd(RT_ifo))
# ezPlot(data = wordagg,
#        dv = RT_ifo,
#        wid = participant,
#        within = .(Validity,Distractor1),
#        between = .(WordPair),
#        x = Distractor1, split = Validity,col = WordPair, do_bars = FALSE)
  
```

Here is a plot that shows the Distribution of valid invalid trials for each word pair
```{r }
wordpairplot <- ggplot(meanWordagg, aes(x=Distractor1, color=Validity, y=RTmean))+
  geom_line(aes(group=Validity, linetype = Validity), size = 1)+
  geom_point(aes(shape = Saliency), size = 3)+
  labs(x="Distractor Word", y = "ReactionTime", colour = "Validity", linetype = "Validity", shape = "Saliency")+
  facet_grid(~WordPair)+
  theme_classic()+
  # ylim(550,600)+
  ggtitle("Distribution of valid and invalid trials of Word pairs")+
  theme(plot.title = element_text(hjust = 0.5))+
  theme(legend.title = element_text(size=14), legend.text= element_text(size=12))+
  theme(axis.title = element_text(size=14),axis.text = element_text(size=12), axis.text.x = element_text(angle = 90))

#presenting the graph
wordpairplot

#ggsave(wordpairplot, file = "Wordpairdistribution.jpeg", dpi = 800)


```

The plot shows the weird pattern where  under each word pair combination (for eg., 1, 2 and so on), one of the word pairs of salient and non salient distractors had faster valid trials compared to invalid trials. 

So to explore this further I split the data into people who had white background compared to people who had black background

### White Background

Here is the data concerning participants with white background: In other words, the participants where the saliency matched with what was coded, and what they saw. Like, Tisch if marked salient, also physically appeared salient in the experiment display 
```{r echo=FALSE, warning=FALSE}
#Selecting only White background participants
whitebg <- Exp1data_OL %>%
  subset(Screen_bg == "white")
table(whitebg$participant)

wordWhitebg <- aggregate(data=whitebg, RT_ifo~participant+Validity+Distractor1+Saliency+WordPair+CorrectAnswer, mean)


wordWhitebg$participant <- as.factor(wordWhitebg$participant)
wordWhitebg$Validity <- as.factor(wordWhitebg$Validity)
wordWhitebg$Distractor1 <- as.factor(wordWhitebg$Distractor1)
wordWhitebg$Saliency <- as.factor(wordWhitebg$Saliency)
wordWhitebg$WordPair <- as.factor(wordWhitebg$WordPair)
wordWhitebg$CorrectAnswer <- as.factor(wordWhitebg$CorrectAnswer)
```

I computed the ANOVA to check for the data trends based on the true assignment and found that there is a significant interaction between Validity and Saliency.The interaction however is not in the expected direction, Still invalid salient trials are faster than valid salient trials
```{r warning=FALSE}
Whitewordanova <- ezANOVA(data = wordWhitebg,
                     dv = RT_ifo,
                     wid = participant,
                     within = .(Validity, Saliency),
                     detailed = TRUE)
anova_out(Whitewordanova)

ezPlot(data = wordWhitebg,
       dv = RT_ifo,
       wid = participant,
       within = .(Validity,Saliency),
       x = Saliency, split = Validity, do_bars = FALSE)


##The pattern is reversed wherein the salient invalid trials are faster than salient valid trials and the opposite for non salient
```

This is the plot that shows the relationship between the words under each combination, only for participants with white background. Inspite of choosing the participants with the right assignment and coding of saliency, the initial pattern where one word pair had faster valid trials than the other still exists.

Interestingly you can see that it is faster for the word pair that is mapped to the right key "l"

Not sure if we can read so much into this but it is a consistent pattern across all combinations so it feels worthy to take note of. 
 
 * Perhaps we may have to check for effect of handedness or wonder whether while doing this study online participants only used one of their hands, instead of how we explain in the lab. 
 

```{r echo=FALSE, warning=FALSE}
meanWordWhite <- ddply(wordWhitebg, .(Validity,Saliency,Distractor1,WordPair,CorrectAnswer), summarize, RTmean=mean(RT_ifo), SD = sd(RT_ifo))

ggplot(meanWordWhite, aes(x=Distractor1, color=Validity, y=RTmean))+
  geom_line(aes(group=Validity, linetype = Validity), size = 1)+
  geom_point(aes(shape = Saliency), size = 3)+
  labs(x="Distractor Word", y = "ReactionTime", colour = "Validity", linetype = "Validity", shape = "Saliency")+
  facet_grid(~WordPair)+
  theme_classic()+
  # ylim(550,600)+
  ggtitle("Distribution of valid and invalid trials of Word pairs for pps with White background")+
  theme(plot.title = element_text(hjust = 0.5))+
  theme(legend.title = element_text(size=14), legend.text= element_text(size=12))+
  theme(axis.title = element_text(size=14),axis.text = element_text(size=12), axis.text.x = element_text(angle = 90))
...
### Black Background
Conducted the same Analysis by choosing only participants with black background.The saliency factor was corrected by interchanging what was labelled as salient and nonsalient. However here there was no main effect or a significant interaction (as in the case for white bg), inspite of correcting for the salience factor assignment.

```{r warning=FALSE}
###Same for Blackbackground

blackbg <- Exp1data_OL %>%
  subset(Screen_bg == "black")
table(blackbg$participant)

blackbg <- blackbg%>%
  mutate(Saliency = ifelse(Saliency == "nonsalient", "salient", "nonsalient"))


wordBlackbg <- aggregate(data=blackbg, RT_ifo~participant+Validity+Distractor1+Saliency+WordPair+CorrectAnswer, mean)


wordBlackbg$participant <- as.factor(wordBlackbg$participant)
wordBlackbg$Validity <- as.factor(wordBlackbg$Validity)
wordBlackbg$Distractor1 <- as.factor(wordBlackbg$Distractor1)
wordBlackbg$Saliency <- as.factor(wordBlackbg$Saliency)
wordBlackbg$WordPair <- as.factor(wordBlackbg$WordPair)
wordBlackbg$CorrectAnswer <- as.factor(wordBlackbg$CorrectAnswer)

Blackwordanova <- ezANOVA(data = wordBlackbg,
                     dv = RT_ifo,
                     wid = participant,
                     within = .(Validity, Saliency),
                     detailed = TRUE)
anova_out(Blackwordanova)

ezPlot(data = wordBlackbg,
       dv = RT_ifo,
       wid = participant,
       within = .(Validity,Saliency),
       x = Saliency, split = Validity, do_bars = FALSE)
...
The pattern here looks different compared to the white bg, but the difference is really small to think of any contributing factors.

Now this is the plot showing the Validity effect for each of the Distractor words and still the same issue is visible inspite of correcting the salience factor. Similar to what was mentioned earlier, it is favourable towards the word pair associated with the Right key

```{r echo=FALSE}

meanWordBlack <- ddply(wordBlackbg, .(Validity,Saliency,Distractor1,WordPair,CorrectAnswer), summarize, RTmean=mean(RT_ifo), SD = sd(RT_ifo))

ggplot(meanWordBlack, aes(x=Distractor1, color=Validity, y=RTmean))+
  geom_line(aes(group=Validity, linetype = Validity), size = 1)+
  geom_point(aes(shape = Saliency), size = 3)+
  labs(x="Distractor Word", y = "ReactionTime", colour = "Validity", linetype = "Validity", shape = "Saliency")+
  facet_grid(~WordPair)+
  theme_classic()+
  # ylim(550,600)+
  ggtitle("Distribution of valid and invalid trials of Word pairs for pps with Black background")+
  theme(plot.title = element_text(hjust = 0.5))+
  theme(legend.title = element_text(size=14), legend.text= element_text(size=12))+
  theme(axis.title = element_text(size=14),axis.text = element_text(size=12), axis.text.x = element_text(angle = 90))
...
### Combined df and recheck ANOVA
Now combing the black and white dataframes to one to have th Original Dataset but with corrected saliency factors for black background participants

```{r}
Expbgchecked <- rbind(whitebg,blackbg)

table(Expbgchecked$Screen_bg,Expbgchecked$Saliency)

Exp1v2agg_fo <- aggregate(data = Expbgchecked, RT_ifo~participant+Saliency+Validity, mean)

#convert to factors for ANOVA
Exp1v2agg_fo$participant <- as.factor(Exp1v2agg_fo$participant)
Exp1v2agg_fo$Saliency <- as.factor(Exp1v2agg_fo$Saliency)
Exp1v2agg_fo$Validity <- as.factor(Exp1v2agg_fo$Validity)

V2aggmeanSV_fo <- ddply(Exp1v2agg_fo, .(Validity,Saliency), summarize, RTmean=mean(RT_ifo), SD = sd(RT_ifo))

#update ANOVA

update_anova_fo <- ezANOVA(data=Exp1v2agg_fo,
                    dv = RT_ifo,
                    wid = participant,
                    within = .(Saliency,Validity),
                    type = 3,
                    detailed = TRUE)
anova_out(update_anova_fo)

ggplot(V2aggmeanSV_fo, aes(x=Saliency, color=Validity, y=RTmean))+
  geom_line(aes(group=Validity, linetype = Validity), size = 1)+
  geom_point(aes(shape = Validity), size = 2)+
  labs(x="Saliency", y = "ReactionTime", colour = "Validity", linetype = "Validity", shape = "Validity")+
  theme_classic()+
  ylim(550,600)+
  ggtitle("Updated Analysis: RT(w.o farouts) as function of saliency and validity")+
  theme(plot.title = element_text(hjust = 0.5))+
  theme(legend.title = element_text(size=14), legend.text= element_text(size=12))+
  theme(axis.title = element_text(size=14),axis.text = element_text(size=12))
...
Exp1v2agg_o <- aggregate(data = Expbgchecked, RT_io~participant+Saliency+Validity, mean)

#convert to factors for ANOVA
Exp1v2agg_o$participant <- as.factor(Exp1v2agg_o$participant)
Exp1v2agg_o$Saliency <- as.factor(Exp1v2agg_o$Saliency)
Exp1v2agg_o$Validity <- as.factor(Exp1v2agg_o$Validity)

V2aggmeanSV_o <- ddply(Exp1v2agg_o, .(Validity,Saliency), summarize, RTmean=mean(RT_io), SD = sd(RT_io))

#update NAOVA

update_anova_o <- ezANOVA(data=Exp1v2agg_o,
                    dv = RT_io,
                    wid = participant,
                    within = .(Saliency,Validity),
                    type = 3,
                    detailed = TRUE)
anova_out(update_anova_o)

ggplot(V2aggmeanSV_o, aes(x=Saliency, color=Validity, y=RTmean))+
  geom_line(aes(group=Validity, linetype = Validity), size = 1)+
  geom_point(aes(shape = Validity), size = 2)+
  labs(x="Saliency", y = "ReactionTime", colour = "Validity", linetype = "Validity", shape = "Validity")+
  theme_classic()+
  ylim(550,600)+
  ggtitle("Updated Analysis: RT(w.o outliers) as function of saliency and validity")+
  theme(plot.title = element_text(hjust = 0.5))+
  theme(legend.title = element_text(size=14), legend.text= element_text(size=12))+
  theme(axis.title = element_text(size=14),axis.text = element_text(size=12))

...
#### Aggregate means with response  relation for previous occurence Analysis 
```{r}
...
Exp1data.agg$ResponseType <- as.factor(Exp1data.agg$ResponseType)
...
####Aggregate for Standard analysis**
...
####ANOVA
# (a) ANOVA Standard analysis
# (RT as function of saliency x validity)

```{r}
anova_ifo <- ezANOVA(data = Standard.agg_ifo,
                    dv = RT_Trial_ifo,
                    wid = Subject,
...
ezStats(data = Standard.agg_ifo,
                    dv = RT_Trial_ifo,
                    wid = Subject,
                    within = .(Saliency,validity))

#Plot
mean_resp_standard <- ezStats(data = Standard.agg_ifo,
                     dv = RT_Trial_ifo,
                     wid = Subject,
                     within = .(Saliency, validity))
...
ggplot(mean_resp_standard, aes(x=Saliency, color=validity, y=Mean))+
  ...
  theme_classic()+
  ylim(450,500)+
  ggtitle("Standard Analysis: RT as function of saliency and validity")+
...
# (b) Previous occurrence analysis
...
ezStats(data = Exp1data.agg,
               dv=RT_Trial_ifo,
               wid=Subject,
               within = .(Saliency, validity, ResponseType))


##Plot three-way interaction (Response Type x Saliency x Validity)
...
ggplot(mean_resp, aes(x=ResponseType, color=validity, y=Mean))+
  ...
  theme_classic()+
  ylim(450,500)+
  ggtitle("Previous occurrence: RT as a function of ResponseType x saliency x validity")+
  theme(plot.title = element_text(hjust = 0.5))+
  theme(legend.title = element_text(size=12), legend.text= element_text(size=10))+
  theme(axis.title = element_text(size=12),axis.text = element_text(size=10))

##Plot saliency x validity interaction

SalxVal <- ezStats(data = Exp1data.agg,
               dv=RT_Trial_ifo,
               wid=Subject,
               within = .(Saliency, validity),

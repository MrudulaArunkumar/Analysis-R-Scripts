...
#load from office
Exp1data_OL <- read.csv("Exp1data_online_fullset.csv")
```
...
###outliers
Exp1agg_o <- aggregate(data = Exp1data_OL, RT_io~participant+Saliency+Validity+ResponseType, mean)


Exp1agg_o2<- ddply(.data = Exp1data_OL, 
                              .variables = c("participant", "Saliency", "Validity", "ResponseType"), 
                              .fun = summarize, 
                              RT.mean=mean(RT_io, na.rm=T))

Exp1agg_o2<-drop_nan(Exp1agg_o$RT.mean)
table(Exp1agg_o2$participant)

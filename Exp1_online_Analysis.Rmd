...
```{r}
#write.csv(Exp1data_OL, file = "D:/PhD/Exp1-Overshadowing/Data/Exp1data_Online_cleaned.csv")
```
...
Checking how the RTs differ between RRs and RCs

```{r Plot based on ResponseType, warning=FALSE}
##FAROUTS
...
##Choosing only the trials where the Distance is 1
...
#Choosing the trials where the distance is 1
Exp1data_OL %>% filter(Distance == 1) %>% 
...
# #bindingAnova <- ezANOVA(Exp1agg_fo,
#                         dv = RT_ifo,
#                         wid = participant,
#                         within = .(Validity, ResponseType),
#                         detailed = TRUE)
# bindmodel <- lmer(RT_ifo~1+ResponseType+(1|Saliency)+(1|Validity)+(1|participant), data = Exp1agg_fo)
# summary(bindmodel)
...
#aggregate means of farouts 
#aggmean_fo <- aggregate(data=Exp1agg_fo, RT_ifo~participant+Saliency+Validity+ResponseType, mean)
#is this different from ezStats? ERROR RELATED TO DESIGN NOT BALANCED
# aggmean_fo <- ezStats(data = Exp1agg_fo,
#                        dv = RT_ifo,
#                        wid = participant,
...
# #bindingAnova <- ezANOVA(Exp1agg_o,
#                         dv = RT_io,
#                         wid = participant,
#                         within = .(Validity, ResponseType),
#                         detailed = TRUE)
# bindmodel <- lmer(RT_io~1+(ResponseType+Saliency+Validity)+(1|participant), data = Exp1agg_o)
# summary(bindmodel)
# anova(bindmodel)
...
```
### 3. Aggregate with error rates
```{r}
Exp1agg_ER <- aggregate(data = Exp1data_OL, ErrorRate~participant+Saliency+Validity, mean)

#Converting to factors for ANOVA
Exp1agg_ER$participant <- as.factor(Exp1agg_ER$participant)
Exp1agg_ER$Saliency <- as.factor(Exp1agg_ER$Saliency)
Exp1agg_ER$Validity <- as.factor(Exp1agg_ER$Validity)
...
#using lmer on the standard dataset and using subjects as random effect --> Not sure if within subject pattern is embedded. Otherwise this model has no random effect
stdmodel <- lmer(RT_ifo~Validity*Saliency + (1|participant), data = Exp1agg_Standard_fo, REML=F)
summary(stdmodel)

#using lmer for mixed effect/multilevel model with random effects as Distance and ResponseType
modelrmdsv <- lmer(RT_ifo~Validity*Saliency+(1|participant)+(1|Distance)+(1|ResponseType), data = Exp1agg_fo, REML = F)
summary(modelrmdsv)
#checking results with BAYES Factor
# Bayesian <- anovaBF(RT_ifo~Saliency*Validity, data = Exp1agg_Standard_fo)
# Bayesian
# plot(Bayesian)

##Checking for Binding effects
# bindingAnova <- ezANOVA(Exp1agg_fo,
#                         dv = RT_ifo,
#                         wid = participant,
#                         within = .(Validity, ResponseType),
...
# bindmodel <- lmer(RT_ifo~1+ResponseType+(1|Saliency)+(1|Validity)+(1|participant), data = Exp1agg_fo)
# summary(bindmodel)
# #BayesBinding <- anovaBF(RT_ifo~Saliency*Validity*ResponseType, data = Exp1agg_fo)
# #BayesBinding
...
#### 2. ANOVA including Distance and Response Type as factors

```{r}
##Throws error because of imbalanced design
# anova_fo <- ezANOVA(Exp1agg_fo,
#                     dv = RT_ifo,
#                     wid = participant,
#                     within = .(Saliency,Validity,ResponseType),
#                     detailed = TRUE)
...
### 3. ANOVA with error rates

```{r}
anova_ER <- ezANOVA(Exp1agg_ER,
                    dv = ErrorRate,
                    wid = participant,
                    within = .(Saliency,Validity),
                    detailed = TRUE)

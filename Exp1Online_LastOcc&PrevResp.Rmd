...
Exp1data_OL <- Exp1data_OL %>% 
  select(-blocks.ran,-blocks.thisIndex,-blocks.thisN,-blocks.thisTrialN)
```
...
Exp1data_OL <- Exp1data_OL %>%
  select(-pracend.keys, -pracend.rt, -Prac_start.rt, -PracRepeat.ran, -PracRepeat.thisIndex, -PracRepeat.thisN, -PracRepeat.thisRepN, -PracRepeat.thisTrialN, -Prac_start.keys, -ResponseKey_p.corr,-ResponseKey_p.keys, -ResponseKey_p.rt)
...
```{r LastOcc condition, include=FALSE}
#creating new column
...
aggDistRespSV <- ddply(Exp1agg_fo, .(Validity,Saliency,ResponseType,Distance), summarize, RTmean=mean(RT_ifo))
...
aggRespSV <- ddply(Exp1agg_Resp_fo, .(Validity,Saliency,ResponseType), summarize, RTmean=mean(RT_ifo))
...
aggRespSV_o<- ddply(Exp1agg_o, .(Validity,Saliency,ResponseType), summarize, RTmean = mean(RT_io))
...
anova_Resp <- aov(RT_ifo~Validity+Saliency+ResponseType+
                       ResponseType*Validity+
                       Validity*Saliency+
                       Saliency*ResponseType+
                       ResponseType*Saliency*Validity,
                     data=Exp1agg_Resp_fo)
...
ggplot(aggRespSV, aes(ResponseType,RTmean,color = Validity)) +
...
anova_Resp_o <- aov(RT_io~Validity+Saliency+ResponseType+
                       ResponseType*Validity+
                       Validity*Saliency+
                       Saliency*ResponseType+
                       ResponseType*Saliency*Validity,
                     data=Exp1agg_Resp_o)
pander(summary(anova_Resp_o),style="rmarkdown",caption = "ANOVA Results of RT(Farouts) x Validity, Saliency and responseType")
```
...
ggplot(aggRespSV_o, aes(ResponseType,RTmean,color = Validity)) +
  geom_line(aes(group=Validity))+
  geom_point()+
  labs(x="Response Type", y = "ReactionTime")+
  ylim(520,620)+
...
anova_DistResp <- aov(RT_ifo~Validity+Saliency+ResponseType+Distance+
                       ResponseType*Validity+
                         ResponseType*Distance+
                       Validity*Saliency+
                         Validity*Distance+
                         Saliency*Distance+
                        ResponseType*Validity*Distance+
                         Saliency*Validity*Distance+
                       Saliency*ResponseType+
                       ResponseType*Saliency*Validity+
                         ResponseType*Saliency*Validity*Distance,
                     data=Exp1agg_fo)
...
anova_DistResp_o <- aov(RT_io~Validity+Saliency+ResponseType+Distance+
                       ResponseType*Validity+
                         ResponseType*Distance+
                       Validity*Saliency+
                         Validity*Distance+
...
### 4. ANOVA with RT and the factors of Validity, Saliency, ResponseType and LastOccurence Condition

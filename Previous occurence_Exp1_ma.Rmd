...
#throws an error because of presence of learn trials
...
Exp1data <- read.csv("Exp1data_Distance+ResponseType_v2.csv")
...
##Aggregate means with response relation
```{r}
Exp1data.agg <- aggregate(data =Exp1data, RT_Trial_ifo~Subject+Saliency+validity+ResponseType+Distance, mean)

#with outliers
Exp1data.agg_io <- aggregate(data =Exp1data, RT_Trial_io~Subject+Saliency+validity+ResponseType+Distance, mean)

#Same df as the one from standard analysis
Exp1data.agg2 <- aggregate(data =Exp1data, RT_Trial_ifo~Subject+Saliency+validity, mean)
...
#saving the aggregate date frame as a csv file for easy access
#write.csv(Exp1data.agg, file = "Exp1.aggdata_RespType.csv")
mean_resp <- ezStats(data = Exp1data.agg,
                     dv = RT_Trial_ifo,
                     wid = Subject,
                     within = .(Saliency, validity, ResponseType))
mean_resp_io <- ezStats(data = Exp1data.agg_io,
                     dv = RT_Trial_io,
                     wid = Subject,
                     within = .(Saliency, validity, ResponseType))
...
anova_ifo <- ezANOVA(data = Exp1data.agg,
                    dv = RT_Trial_ifo,
                    wid = Subject,
                    within = .(Saliency,validity),
                    detailed = TRUE)
...
mean_resp_standard <- ezStats(data = Exp1data.agg,
                      dv = RT_Trial_ifo,
                      wid = Subject,
                      within = .(Saliency, validity))


mean_resp_standard_io <- ezStats(data = Exp1data.agg_io,
                      dv = RT_Trial_io,
                      wid = Subject,
                      within = .(Saliency, validity))

anova_io <- ezANOVA(data = Exp1data.agg_io,
                    dv = RT_Trial_io,
                    wid = Subject,
                    within = .(Saliency,validity),
                    detailed = TRUE)
...
#Accounting for previous occurences with farouts
ezout_ifo<-ezANOVA(data = Exp1data.agg,
               dv=RT_Trial_ifo,
               wid=Subject,
               within = .(Saliency, validity, ResponseType),
               detailed = TRUE)
  ...
  #with outliers
  ezout_io<-ezANOVA(data = Exp1data.agg_io,
               dv=RT_Trial_io,
               wid=Subject,
               within = .(Saliency, validity, ResponseType),
               detailed = TRUE)
...
ggplot(mean_resp_standard, aes(x=Saliency, color=validity, y=Mean))+
  ...
  theme(axis.title = element_text(size=12),axis.text = element_text(size=10))
#outliers
ggplot(mean_resp_standard_io, aes(x=Saliency, color=validity, y=Mean))+
  geom_line(aes(group=validity))+
  geom_point()+
  labs(x="Saliency", y = "ReactionTime")+
  theme_classic()+
...
ggplot(mean_resp, aes(x=ResponseType, color=validity, shape = Saliency, y=Mean))+
  geom_line(aes(group=validity,Saliency))+
  geom_point()+
  labs(x="Response Type", y = "ReactionTime")+
  theme_classic()+
  ylim(450,500)+
